...
let UserName = "";
let NumberUser = "";
let AllsInputToDoSemthingAction = "";
let valToSereach = "";
...
let ModeleSereach = "";
let datUserClick = "";
export default function AddCelyaneForMehale() {
  ...
  React.useMemo(() => {
    if (typeRequestRsp === "edartzebayenstoaddzebouneyedewiyanwithbss") {
      ...
      if (resultrquestaction === 1) {
        OpenDialogForActionSuccess(
          "تم انشاء زبون يدويا بنجاح بعد قليل سيتم توجيهك الى صفحت ادارة زباين",
          "active"
        );
        setTimeout(() => {
          window.location.href = "/my-Zebayn";
        }, 4500);
  ...
  React.useMemo(() => {
    ...
    AllsInputToDoSemthingAction = dataShowTable.map((ShowDat) => {
      return (
        <div
          style={{
            marginBlock: "16px",
