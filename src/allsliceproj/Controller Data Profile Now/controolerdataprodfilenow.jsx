  ...
  async (dat, { rejectWithValue }) => {
    ...
    try {
      const response = await axios.post(
        ...
        {}, // body فارغ إذا لم تكن بحاجة لإرسال بيانات
        {
          headers: {
...
export const startshngeprofileusernowtologin = createAsyncThunk(
  ...
  async (QuaryData, { rejectWithValue }) => {
    ...
    try {
      ...
      const datprofNow = response.data.data.original.Profilenow;
      const AllsDataProfNow = response.data.data.original;
      return {
        datprofNow,
        AllsDataProfNow,
      };
...
const controolerdataprodfilenow = createSlice({
...
export const { testreducer } = controolerdataprodfilenow.actions;
export default controolerdataprodfilenow.reducer;
