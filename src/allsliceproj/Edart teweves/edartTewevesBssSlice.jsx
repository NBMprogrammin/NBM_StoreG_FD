...
export const edarttewevebsstogetratibeformeweve = createAsyncThunk(
  ...
  async (QuatyData) => {
    try {
      const response = await axios.post(
        `${GeneralUrlGetDatabase}/bss/EdartMeweve-Get-Ratibe/${QuatyData.data.id}`,
        QuatyData,
        {
          headers: {
            Authorization: "Bearer " + tokenFoul,
...
const edartTewevesBssSlice = createSlice({
  name: "edartmewvesbss",
  initialState: {
    data: [],
    isLinding: false,
...
export const { testreducer } = edartTewevesBssSlice.actions;
export default edartTewevesBssSlice.reducer;
